...
def run_all_experiments(multi_threaded):
    ...
    for file_name in experiments:
        ...
        run_string = ['python3 ../tool/main.py ' + experiment + ' -stat -m -d > /dev/null', experiment]
        q.put(run_string)
        run_string = ['python3 ../tool/main.py ' + experiment + ' -stat -m -d -n > /dev/null', experiment]
        q.put(run_string)
    if multi_threaded:
...
def run_timing_experiments(multi_threaded):
    ...
    for file_name in experiments:
        experiment = experiment_dir + file_name
        run_string = ['python3 ../tool/main.py ' + experiment + ' -tstat -na -v > /dev/null', experiment]
        q.put(run_string)
        if 'Linux_BSDW' not in experiment and 'sqlite_MEMFW' not in experiment:
            run_string = ['python3 ../tool/main.py ' + experiment + ' -tstat -na -v -n > /dev/null', experiment]
            q.put(run_string)
    if multi_threaded:
        cpus = multiprocessing.cpu_count()
    else:
